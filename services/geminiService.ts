...
export const generateThreadsContent = async (request: GenerateRequest): Promise<ThreadPost[]> => {
  ...
  const prompt = `
    ...
    3.  **補充主題**: ${request.customTopic || "無特定主題，自由發揮"}
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: "You are a creative writer for social media, specializing in the 'Threads' app style which is casual, authentic, and sometimes slightly unhinged or very relatable.",
